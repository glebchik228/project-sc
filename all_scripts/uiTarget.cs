...
public class uiTarget : MonoBehaviour
{
	...
	{
		Vector3 realPos = mainCamera.WorldToScreenPoint(Target[target].position); // получениее экранных координат объекта
		...
		if (!IsBehind(Target[target].position)) // если цель спереди
		{
		...
		else // если цель cзади
		{
			...
			if (mainCamera.transform.position.y < Target[target].position.y)
			{
				direction = -1;
